...
namespace PowerSystem
{
	...
	void BusAndBranch::FromMemory(ExBus **buses, int bun, ExBranch ** branches, int bran)
	{
		for (int i = 0; i < bun; i++)
		{
			...
			bus.push_back(Bus(buses[i]->i, buses[i]->name, (Bus::NodeType)(int)buses[i]->type, buses[i]->V, buses[i]->cita, 1.0, buses[i]->GP, buses[i]->GQ, buses[i]->LP , buses[i]->LQ, buses[i]->Qmax, buses[i]->Qmin, buses[i]->b));
		}

		for (int i = 0; i < bran; i++)
		{
			//Branch(int _i, int _j, string _Name, BranchType _Type, double _R, double _X, double _B, double _K)
			branch.push_back(Branch(branches[i]->getI(), branches[i]->getJ(), branches[i]->getName(), (Branch::BranchType)(int)branches[i]->getType(), branches[i]->getR(), branches[i]->getX(), branches[i]->getB(), branches[i]->getK()));
		}
	}
	...
	void _PowerFlow::InitBaBFromMemory(Bus **buses, int bun, Branch ** branches, int bran)
	{
		BAB.FromMemory(buses, bun, branches, bran);
		Y.reserve(BAB.GetBusSize() + 2 * BAB.GetBranchSize());
		NodeIdMap.reserve(BAB.GetBusSize());
	}

	void _PowerFlow::ToMemory(ExBus ** buses)
	{
		for (int i=0; i<BAB.GetBusSize(); i++)
		{
			BusAndBranch::Bus * pBu = BAB.GetBus(i);

			buses[i]->V = pBu->V;
			buses[i]->cita = pBu->cita;
			if (buses[i]->getModelType() == ExBus::GENERATOR)
			{
				buses[i]->GP = pBu->P;
				buses[i]->GQ = pBu->Q;
			} else {
		...
		for (vector<Pair>::const_iterator it = NodeIdMap.begin(); it != NodeIdMap.end(); it++)
		{
			...
			for (int i = 0; i < BAB.GetBranchSize(); i++)
			{
				...
				if (pBr->i == realId || pBr->j == realId)
				{
					...
					Y[major].data += ym + complex<double>(0.0, pBr->B) / 2.0;
